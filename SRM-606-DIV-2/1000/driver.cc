...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> sweets;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << sweets << std::endl;
		std::cout.flush();

		__timer_start();

		EllysCandyGame __object;
		string __actual = __object.getWinner(sweets);
