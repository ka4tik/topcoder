...
int	K;
		...
		__in >> __expected >> kind >> found >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << kind << ", " << found << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		Excavations2 __object;
		long long __actual = __object.count(kind, found, K);
