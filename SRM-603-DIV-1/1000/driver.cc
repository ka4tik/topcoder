...
vector <int>	Bseed;
		...
		__in >> __expected >> n >> Aseed >> Bseed;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << Aseed << ", " << Bseed << std::endl;
		std::cout.flush();

		__timer_start();

		SumOfArrays __object;
		string __actual = __object.findbestpair(n, Aseed, Bseed);
