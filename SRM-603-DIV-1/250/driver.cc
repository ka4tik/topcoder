...
vector <int>	costs;
		...
		__in >> __expected >> edges >> costs;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << edges << ", " << costs << std::endl;
		std::cout.flush();

		__timer_start();

		MaxMinTreeGame __object;
		int __actual = __object.findend(edges, costs);
