...
vector <int>	taste;
		...
		__in >> __expected >> type >> taste;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << type << ", " << taste << std::endl;
		std::cout.flush();

		__timer_start();

		AlienAndHamburgers __object;
		int __actual = __object.getNumber(type, taste);
