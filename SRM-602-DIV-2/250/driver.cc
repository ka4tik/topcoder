...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> rating;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << rating << std::endl;
		std::cout.flush();

		__timer_start();

		TypoCoderDiv2 __object;
		int __actual = __object.count(rating);
