...
int	limit;
		...
		__in >> __expected >> X >> Y >> limit;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << X << ", " << Y << ", " << limit << std::endl;
		std::cout.flush();

		__timer_start();

		PilingRectsDiv2 __object;
		int __actual = __object.getmax(X, Y, limit);
