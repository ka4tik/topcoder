...
string	side;
		...
		__in >> __expected >> front >> side;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << front << ", " << side << std::endl;
		std::cout.flush();

		__timer_start();

		BlackBoxDiv2 __object;
		int __actual = __object.count(front, side);
