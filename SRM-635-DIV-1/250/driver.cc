...
vector <int>	rating;
		...
		__in >> __expected >> date >> rating;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << date << ", " << rating << std::endl;
		std::cout.flush();

		__timer_start();

		SimilarRatingGraph __object;
		double __actual = __object.maxLength(date, rating);
