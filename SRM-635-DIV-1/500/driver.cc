...
vector <int>	cutoff;
		...
		__in >> __expected >> places >> cutoff;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << places << ", " << cutoff << std::endl;
		std::cout.flush();

		__timer_start();

		StoryFromTCO __object;
		int __actual = __object.minimumChanges(places, cutoff);
