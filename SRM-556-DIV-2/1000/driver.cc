...
#include "LeftRightDigitsGame.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> digits;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << digits << std::endl;
		std::cout.flush();

		__timer_start();

		LeftRightDigitsGame __object;
		string __actual = __object.minNumber(digits);
