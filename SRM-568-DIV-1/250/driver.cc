...
vector <int>	blue;
		...
		__in >> __expected >> red >> green >> blue;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << red << ", " << green << ", " << blue << std::endl;
		std::cout.flush();

		__timer_start();

		BallsSeparating __object;
		int __actual = __object.minOperations(red, green, blue);
