...
int main(int argc, char* argv[])
{
	...
	__in >> __expected >> seed >> mul >> add >> N >> K;
	if (!__in.good()) break;

	std::cout << "# input for test " << __testnum << ": " << seed << ", " << mul << ", " << add << ", " << N << ", " << K << std::endl;
	std::cout.flush();

	__timer_start();

	FloatingMedian __object;
	long long __actual = __object.sumOfMedians(seed, mul, add, N, K);
