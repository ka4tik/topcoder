...
#include "YetAnotherTwoTeamsProblem.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> skill;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << skill << std::endl;
		std::cout.flush();

		__timer_start();

		YetAnotherTwoTeamsProblem __object;
		long long __actual = __object.count(skill);
