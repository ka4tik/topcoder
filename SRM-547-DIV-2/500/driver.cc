...
int	w;
		...
		__in >> __expected >> height >> w;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << height << ", " << w << std::endl;
		std::cout.flush();

		__timer_start();

		PillarsDivTwo __object;
		double __actual = __object.maximalLength(height, w);
