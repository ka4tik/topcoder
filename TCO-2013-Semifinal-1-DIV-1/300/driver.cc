...
int	maxC;
		...
		__in >> __expected >> minR >> maxR >> minC >> maxC;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << minR << ", " << maxR << ", " << minC << ", " << maxC << std::endl;
		std::cout.flush();

		__timer_start();

		RabbitsAndCakes __object;
		long long __actual = __object.getNumber(minR, maxR, minC, maxC);
