...
vector <string>	allC;
		...
		__in >> __expected >> allA >> allB >> allC;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << allA << ", " << allB << ", " << allC << std::endl;
		std::cout.flush();

		__timer_start();

		WinterAndReindeers __object;
		int __actual = __object.getNumber(allA, allB, allC);
