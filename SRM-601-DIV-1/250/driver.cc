...
vector <int>	orange;
		...
		__in >> __expected >> apple >> orange;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << apple << ", " << orange << std::endl;
		std::cout.flush();

		__timer_start();

		WinterAndPresents __object;
		long long __actual = __object.getNumber(apple, orange);
