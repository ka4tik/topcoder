...
int	r;
		...
		__in >> __expected >> board >> r;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << board << ", " << r << std::endl;
		std::cout.flush();

		__timer_start();

		ClosestRabbit __object;
		double __actual = __object.getExpected(board, r);
