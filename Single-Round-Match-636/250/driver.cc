...
#include "ChocolateDividingEasy.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> chocolate;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << chocolate << std::endl;
		std::cout.flush();

		__timer_start();

		ChocolateDividingEasy __object;
		int __actual = __object.findBest(chocolate);
