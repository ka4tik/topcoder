...
#include "GraphWalkWithProbabilities.cc"
...
int	Start;
		...
		__in >> __expected >> graph >> winprob >> looseprob >> Start;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << graph << ", " << winprob << ", " << looseprob << ", " << Start << std::endl;
		std::cout.flush();

		__timer_start();

		GraphWalkWithProbabilities __object;
		double __actual = __object.findprob(graph, winprob, looseprob, Start);
