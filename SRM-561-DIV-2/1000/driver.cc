...
#include "FoxAndTouristFamilies.cc"
...
vector <int>	f;
		...
		__in >> __expected >> A >> B >> f;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << A << ", " << B << ", " << f << std::endl;
		std::cout.flush();

		__timer_start();

		FoxAndTouristFamilies __object;
		double __actual = __object.expectedLength(A, B, f);
