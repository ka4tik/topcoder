...
#include "TheProgrammingContestDivOne.cc"
...
vector <int>	requiredTime;
		...
		__in >> __expected >> T >> maxPoints >> pointsPerMinute >> requiredTime;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << T << ", " << maxPoints << ", " << pointsPerMinute << ", " << requiredTime << std::endl;
		std::cout.flush();

		__timer_start();

		TheProgrammingContestDivOne __object;
		int __actual = __object.find(T, maxPoints, pointsPerMinute, requiredTime);
