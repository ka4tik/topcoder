...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> radius;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << radius << std::endl;
		std::cout.flush();

		__timer_start();

		MarblePositioning __object;
		double __actual = __object.totalWidth(radius);
