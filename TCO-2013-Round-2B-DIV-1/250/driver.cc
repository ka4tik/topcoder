...
int	grape;
		...
		__in >> __expected >> apple >> kiwi >> grape;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << apple << ", " << kiwi << ", " << grape << std::endl;
		std::cout.flush();

		__timer_start();

		FruitTrees __object;
		int __actual = __object.maxDist(apple, kiwi, grape);
