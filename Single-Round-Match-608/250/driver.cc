...
#include "OneDimensionalRobotEasy.cc"
...
int	B;
		...
		__in >> __expected >> commands >> A >> B;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << commands << ", " << A << ", " << B << std::endl;
		std::cout.flush();

		__timer_start();

		OneDimensionalRobotEasy __object;
		int __actual = __object.finalPosition(commands, A, B);
