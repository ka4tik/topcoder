...
#include "MysticAndCandiesEasy.cc"
...
vector <int>	high;
		...
		__in >> __expected >> C >> X >> high;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << C << ", " << X << ", " << high << std::endl;
		std::cout.flush();

		__timer_start();

		MysticAndCandiesEasy __object;
		int __actual = __object.minBoxes(C, X, high);
