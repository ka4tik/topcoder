...
vector <int>	s;
		...
		__in >> __expected >> N >> K >> s;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << K << ", " << s << std::endl;
		std::cout.flush();

		__timer_start();

		ShoppingSurveyDiv1 __object;
		int __actual = __object.minValue(N, K, s);
