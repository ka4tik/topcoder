...
vector <int>	y;
vector <int>	redScore;
vector <int>	blueScore;

		__in >> __expected >> x >> y >> redScore >> blueScore;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << x << ", " << y << ", " << redScore << ", " << blueScore << std::endl;
		std::cout.flush();

		__timer_start();

		SegmentDrawing __object;
		int __actual = __object.maxScore(x, y, redScore, blueScore);
