...
#include "EllysNumberGuessing.cc"
...
vector <int>	answers;
		...
		__in >> __expected >> guesses >> answers;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << guesses << ", " << answers << std::endl;
		std::cout.flush();

		__timer_start();

		EllysNumberGuessing __object;
		int __actual = __object.getNumber(guesses, answers);
