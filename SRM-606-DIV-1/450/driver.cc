...
vector <int>	add;
		...
		__in >> __expected >> M >> count >> first >> mult >> add;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << M << ", " << count << ", " << first << ", " << mult << ", " << add << std::endl;
		std::cout.flush();

		__timer_start();

		EllysPairing __object;
		int __actual = __object.getMax(M, count, first, mult, add);
