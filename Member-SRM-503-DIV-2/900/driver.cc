...
#include "KingdomXCitiesandVillagesAnother.cc"
...
vector <int>	villageY;
		...
		__in >> __expected >> cityX >> cityY >> villageX >> villageY;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << cityX << ", " << cityY << ", " << villageX << ", " << villageY << std::endl;
		std::cout.flush();

		__timer_start();

		KingdomXCitiesandVillagesAnother __object;
		double __actual = __object.determineLength(cityX, cityY, villageX, villageY);
