...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> numbers;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << numbers << std::endl;
		std::cout.flush();

		__timer_start();

		EllysCoprimesDiv2 __object;
		int __actual = __object.getCount(numbers);
