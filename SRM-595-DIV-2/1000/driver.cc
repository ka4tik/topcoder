...
#include "LittleElephantAndXor.cc"
...
int	C;
		...
		__in >> __expected >> A >> B >> C;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << A << ", " << B << ", " << C << std::endl;
		std::cout.flush();

		__timer_start();

		LittleElephantAndXor __object;
		long long __actual = __object.getNumber(A, B, C);
