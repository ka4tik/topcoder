...
vector <int>	price;
		...
		__in >> __expected >> dread >> price;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << dread << ", " << price << std::endl;
		std::cout.flush();

		__timer_start();

		MonstersValley __object;
		int __actual = __object.minimumPrice(dread, price);
