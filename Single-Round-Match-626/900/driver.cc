...
#include "ReflectiveRectangle.cc"
...
int	bounces;
		...
		__in >> __expected >> sideA >> sideB >> bounces;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << sideA << ", " << sideB << ", " << bounces << std::endl;
		std::cout.flush();

		__timer_start();

		ReflectiveRectangle __object;
		int __actual = __object.findSum(sideA, sideB, bounces);
