...
int	d;
		...
		__in >> __expected >> a >> b >> c >> d;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << a << ", " << b << ", " << c << ", " << d << std::endl;
		std::cout.flush();

		__timer_start();

		FixedDiceGameDiv1 __object;
		double __actual = __object.getExpectation(a, b, c, d);
