...
int	d;
		...
		__in >> __expected >> isFriend >> d;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << isFriend << ", " << d << std::endl;
		std::cout.flush();

		__timer_start();

		Egalitarianism __object;
		int __actual = __object.maxDifference(isFriend, d);
