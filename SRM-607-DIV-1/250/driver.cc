...
#include "PalindromicSubstringsDiv1.cc"
...
vector <string>	S2;
		...
		__in >> __expected >> S1 >> S2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << S1 << ", " << S2 << std::endl;
		std::cout.flush();

		__timer_start();

		PalindromicSubstringsDiv1 __object;
		double __actual = __object.expectedPalindromes(S1, S2);
